...
export class GoogleDriveAdapter {
     ...
     */
    async openSession(source, accessToken) {
        const metadata = {
     ...
     */
    async queryProgress(session, accessToken) {
        const ctrl = new AbortController();
        const timeout = setTimeout(() => ctrl.abort(), this.config.requestTimeoutMs);
        try {
     ...
     */
    async uploadChunk(session, chunk, accessToken, expectedStart) {
        ...
        try {
            ...
            if (res.status === 308) {
                ...
                if (range) {
                    const m = /bytes=0-(\d+)/.exec(range);
                    if (m)
                        return { acknowledged: Number(m[1]) + 1, driveFileId: null, finished: false };
                }
            ...
            if (res.status === 200 || res.status === 201) {
                ...
                return {
                    ...
                    driveFileId: (payload && payload.id) || null,
                    finished: true,
                };
            }
...
//# sourceMappingURL=GoogleDriveAdapter.js.map