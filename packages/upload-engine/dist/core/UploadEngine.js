...
export class UploadEngine {
    ...
    constructor(userConfig) {
        ...
        this.scheduler = new UploadScheduler(this.config, this.queue, this.sessions, this.retry, this.adapter, this.progress, this.logger, {
            onFileStateChange: (id) => {
                const snap = this.queue.snapshot(id);
                if (snap)
                    this.config.onFileStateChange?.(snap);
            },
        });
    }
    /**
    ...
    resumeFile(id) {
        ...
        if (!f)
            return;
        if (f.state !== "paused" && f.state !== "failed")
            return;
